...
//  Main Code //   
    ...
    if(lcdMain != null){     
        ...
        Current = new GPSlocation("test",remote.GetPosition());   
        string store = Current.ToString();   
...
// Utility Methods 
    ...
    // Set Variables      
        ...
        // BlackBox Storage in Gyroscopes 
            writeToLine(lcdMain,(runCount + "- BlackBox.Count: " + blackbox.Count),true);  
            GridTerminalSystem.GetBlocksOfType<IMyGyro>(l3);  
            writeToLine(lcdMain,("Gyroscopes: " + l3.Count + "\r\n"),true); 
            ...
            if(l3.Count != 0){ 
                for(int i = 0; i < l3.Count;i++){ 
                    ...
                    if(sData[0] == "Storage Index"){ 
                        int j; 
                        int storageCount = Int32.TryParse(sData[1], out j) ? j : -1;    
                        if(storageCount != -1){
                            writeToLine(lcdMain,("BlackBox.Insert(" + storageCount + ", " + l3[i].CustomName + ")\r\n"),true);
                            blackbox.Insert(storageCount,(IMyGyro)l3[i]);
                        }else{
                            blackbox.Add((IMyGyro)l3[i]);
                            writeToLine(lcdMain,("BlackBox.Add(" + l3[i].CustomName + ") - Line 125\r\n"),true);
                        } 
                    }else{ 
                        blackbox.Add((IMyGyro)l3[i]);
                        writeToLine(lcdMain,("BlackBox.Add(" + l3[i].CustomName + ") - Line 129\r\n"),true); 
                        l3[i].CustomData = "";
                    } 
                }  

                writeToLine(lcdMain,("BlackBox.Count: " + blackbox.Count),true);       
            }

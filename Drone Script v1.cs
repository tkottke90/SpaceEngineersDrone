...
}catch(Exception e){writeToLine(lcdMain,e.ToString(),true);Echo(e.ToString());}
}               
...
// Utility Methods           
    ...
    // Set Variables                
        ...
        // Other LCDs:           
            ...
            */           
            ...
            foreach(IMyTextPanel txt in l1){            
                ...
                if(DEBUG){writeToLine(lcdMain,("148 - LCD Panel (txt) = " + txt.CustomName),true);}
        ...
        if(DEBUG){writeToLCD(lcdMain,"194 - Set Variables Initalized",true);} 
        return true;           
    }           
...
// LCD Methods             
    ...
    public string drawLCDStatus(IMyTextPanel lcd){            
        ...
        if(DEBUG){writeToLine(lcdMain,("287 - LCD Panel (lcd) = " + lcd.CustomName),true);} 
    ...
    public string matchTextSpace(string[] strPrime, string[] strBeta, IMyTextPanel lcd){      
        ...
        if(DEBUG){
            writeToLine(lcdMain,("362 - LCD Panel (lcd) = " + lcd.CustomName),true);
            writeToLine(lcdMain,("362 - strPrime = " + String.Join("|",strPrime)),true);
            writeToLine(lcdMain,("362 - strBeta = " + String.Join("|",strBeta)),true);
        } 

        for(int i = 0; i < strBeta.Length; i++){      
            output += strBeta[i];      
            if(DEBUG){writeToLine(lcdMain,("370 - " + i + " - " + output),true);}     
            float dataSpacer = (strPrime[i].Length + spacerCount) - strBeta[i].Length;      
            for(int j = 0; j < dataSpacer; j++){
                if(DEBUG){writeToLine(lcdMain,("373 - " + output),true);}
                output += spacer;
            }      
        }      
        
        if(DEBUG){writeToLine(lcdMain,("375 - " + output),true);}
        return output;     
    }      

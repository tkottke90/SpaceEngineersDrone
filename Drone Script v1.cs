        ...
        case "Idle":            
            ...
            // Create new waypoint -> Check records for waypoint -> if new waypoint, investigate           
                int selector = 0; for(int i = 1; i < 6; i++){     
                    if(DEBUG){writeToLine(lcdMain,("56 - \r\n \tSelector = " + selector + "\r\n \ti = " + i),true);for(int j = 0; j < AI.aiFitness.Count; j++){writeToLine(lcdMain,("AI Fitness Key: " + j + " : Value = " + AI.aiFitness[j]),true);}} 
                    if(AI.aiFitness[i] > AI.aiFitness[selector]){selector = i;} 
                }     
                newCoordinate(selector);     
...
// Utility Methods            
    ...
    public void getPreferences(AIModule a){                
        ...
        if(DEBUG){writeToLCD(lcdMain,"258 - Get Preferences Initalized",true);}  
    }              
...
// LCD Methods              
    ...
    public string drawLCDStatus(IMyTextPanel lcd){             
        ...
        if(LCD_DEBUG){writeToLine(lcdMain,("310 - LCD Panel (lcd) = " + lcd.CustomName),true);}  
    ...
    public string matchTextSpace(string[] strPrime, string[] strBeta, IMyTextPanel lcd){       
        ...
        if(LCD_DEBUG){ 
            writeToLine(lcdMain,("386 - LCD Panel (lcd) = " + lcd.CustomName),true); 
            writeToLine(lcdMain,("387 - strPrime = " + String.Join("|",strPrime)),true); 
            writeToLine(lcdMain,("388 - strBeta = " + String.Join("|",strBeta)),true); 
        }  
 
        for(int i = 0; i < strBeta.Length; i++){       
            output += strBeta[i];       
            if(LCD_DEBUG){writeToLine(lcdMain,("394 - " + i + " - " + output),true);}      
            float dataSpacer; 
            try{ 
                dataSpacer = (strPrime[i].Length + spacerCount) - strBeta[i].Length;       
            }catch(Exception e){Echo("395 - " + e.ToString());dataSpacer = 10.0f;} 
            for(int j = 0; j < dataSpacer; j++){ 
                if(LCD_DEBUG){writeToLine(lcdMain,("396 - " + output),true);} 
                output += spacer; 
            }       
        }       
         
        if(LCD_DEBUG){writeToLine(lcdMain,("375 - " + output),true);} 
        return output;      
    }       
...
// Navigation Methods            
    ...
    public GPSlocation newCoordinate(int selector){   
        ...
        switch(selector){   
            case 1:   
                return new GPSlocation("test",new Vector3D(0,0,0));  
                break;   
        }   
